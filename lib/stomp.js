...
Stomp.prototype.should_run_message_callback = function(_frame) {
    ...
    var subscription = this._subscribed_to[_frame.headers.subscription] ||
        this._subscribed_to[_frame.headers.destination];
...
Stomp.prototype.subscribe = function(headers, callback) {
    ...
    if (destination instanceof Array) {
        for (var i = 0; i < destination.length; i++) {
            ...
            this.log.debug('subscribed to: ' + headers.destination + ' with headers ' + JSON.stringify(headers));
        }
    }
    // subscribe to one queues
    else {
        send_command(this, 'SUBSCRIBE', headers);
        this.log.debug('subscribed to: [' + destination + '] with headers ' + JSON.stringify(headers));
    }

    this._subscribed_to[id] = { enabled: true, callback: callback };
};
...
Stomp.prototype.unsubscribe = function(headers) {
    ...
    this._subscribed_to[id || destination].enabled = false;
